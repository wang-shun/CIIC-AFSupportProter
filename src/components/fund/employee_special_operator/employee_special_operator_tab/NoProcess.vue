<template>
  <div>
    ...
    <Row class="mt20">
      <Col :sm="{span:24}">
        <Table border :columns="noProcessColumns" :data="data.sNoProcessData"></Table>
        ...
        <Row>
          ...
          <Col :sm="{span: 12}">
            <Form-item label="变更类型：">
              <Select v-model="createTaskTicket.changeTypeValue" style="width: 100%;" transfer>
                <Option v-for="item in createTaskTicket.changeTypeList" :value="item.value" :key="item.value">{{item.label}}</Option>
              </Select>
            </Form-item>
          </Col>
        </Row>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        createTaskTicket: {
          ...
          createColumns: [
            ...
            {title: '公司编码', key: 'companyNumber', align: 'center',
              render: (h, params) => {
                return h('div', {style: {textAlign: 'left'}}, [
    ...
    methods: {
      ...mapActions('sNoProcess',[EventType.SNOPROCESS]),
