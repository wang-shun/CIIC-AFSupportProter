    ...
    <Form>
      <Row class="mt20">
        <Col :xs="{span: 24}" :lg="{span: 24}">
          <Table border :columns="taskColumns" :data="cfinished.taskData"></Table>
      ...
      <Modal
        ...
        @on-ok="ok"
        @on-cancel="cancel">
        <customer-modal :customerData="cfinished.customerData"></customer-modal>
      </Modal>
    </Form>
  </div>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('cFinished',[
        'cfinished'
      ])
    },
    methods: {
      ...mapActions('cFinished',{
        setCNextMonthHandle: eventType.CFINISHEDTYPE
      }),
