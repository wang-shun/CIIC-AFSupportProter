  ...
  <div>
    ...
    <Row class="mt20">
      <Col :xs="{span: 24}" :lg="{span: 24}">
        <Table border :columns="employeeResultColumns" :data="finishhandle.employeeResultData"></Table>
    ...
    <Modal
      ...
      @on-ok="ok"
      @on-cancel="cancel">
      <customer-modal :customerData="finishhandle.customerData"></customer-modal>
    </Modal>
    ...
    <Modal
      ...
      @on-ok="ok"
      @on-cancel="cancel">
      <company-account-search-modal :sSocialSecurityTypeData="finishhandle.sSocialSecurityTypeData"></company-account-search-modal>
    </Modal>
  </div>
</template>
<script>
  ...
  export default {
    ...
    mounted() {
      this.setFinishHandle()
    },
    computed: {
      ...mapGetters('finishHandle',[
        'finishhandle'
      ])
    },
    methods: {
      ...mapActions('finishHandle',{
        setFinishHandle: eventType.FINISHHANDLETYPE
      }),
