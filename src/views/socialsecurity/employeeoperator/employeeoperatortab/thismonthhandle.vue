<template>
  <div>
              ...
              <Col :sm="{span:22}" :md="{span: 12}" :lg="{span: 8}">
                <Form-item label="*未处理：" prop="handleMonth">
                  <Select v-model="operatorSearchData.handleMonth" style="width: 100%;" transfer>
                    <Option v-for="item in operatorSearchData.handleMonthList" :value="item.value" :key="item.value">{{item.label}}</Option>
                  </Select>
                </Form-item>
              </Col>
              <Col :sm="{span:22}" :md="{span: 12}" :lg="{span: 8}">
    ...
    <Row class="mt20">
      <Col :sm="{span:24}">
        <Table border ref="selection" :columns="employeeResultColumns" :data="data.employeeResultData"></Table>
